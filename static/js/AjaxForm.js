        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (buttonName === 'submit_opening' && response) {
                    ...
                    $('#openingText2').text(response.opening_text_2 || 'Ssan');
                    $('#openingText3').text(response.opening_text_3 || 'Dan');
                    $('#openingText4').text(response.opening_text_4 || 'Aqila');
                ...
                if (buttonName === 'submit_greeting' && response) {
                    ...
                    $('#greetingText2').text(response.greeting_text_2 || 'Tanpa mengurangi rasa hormat kami bermaksud mengundang Bapak/Ibu/Saudara/i pada acara resepsi pernikahan kami');
                ...
                if (buttonName === 'submit_quote' && response) {
                    ...
                    $('#quotesText3').text(response.quotes_text_3 || '(QS. Ar-Ruum 30 : 21)');
                ...
                if (buttonName === 'submit_mempelai' && response) {
                    ...
                    $('#mempelaiText1Pria').text(response.mempelai_text_1_pria || 'Hassan Yusuf');
                    $('#mempelaiText2Pria').text(response.mempelai_text_2_pria || 'Putra Pertama Bapak H. Yusuf dan Ibu Hj. Shafira');
                    $('#mempelaiText3Pria').text(response.mempelai_text_3_pria || '@username');
                    ...
                    $('#mempelaiText1Wanita').text(response.mempelai_text_1_wanita || 'Salsabila Aqila');
                    $('#mempelaiText2Wanita').text(response.mempelai_text_2_wanita || 'Putri Kedua Bapak Ir. Imran Naufal dan Ibu Hana Sofia');
                    $('#mempelaiText3Wanita').text(response.mempelai_text_3_wanita || '@username');
                    ...
                    const defaultImage = '/media/logo/blank-pic-removebg.png';

                    // Update gallery images
                    if (response.mempelai_images_pria) {
                        $('#mempelai_images_pria_preview').attr('src', response.mempelai_images_pria);
                    } else {
                        $('#mempelai_images_pria_preview').attr('src', defaultImage);
                    }

                    if (response.mempelai_images_wanita) {
                        $('#mempelai_images_wanita_preview').attr('src', response.mempelai_images_wanita);
                    } else {
                        $('#mempelai_images_wanita_preview').attr('src', defaultImage);
                    }
                ...
                if (buttonName === 'submit_akad' && response) {
                    ...
                    $('#akadText1').text(response.akad_text_1 || 'Akad Nikah');
                    $('#eventDetailsAkad').html(
                        (response.tanggal_akad ? formatDate(response.tanggal_akad) : 'Minggu, 2 Januari 2025') + '<br>' +
                        'Pukul ' + (response.waktu_akad ? formatTime(response.waktu_akad)  :  '10:00')+ ' '   +
                        (response.zona_waktu_akad || 'WITA') + '<br>' +
                        (response.alamat_akad || 'Inviteku.com') + '<br>' +
                        (response.lokasi_akad || 'Jl. Anggrek Kencana, Luwu Raya, Sulawesi Selatan, Indonesia')
                    );
                ...
                if (buttonName === 'submit_resepsi' && response) {
                    ...
                    $('#resepsiText1').text(response.resepsi_text_1 || 'Resepsi');
                    $('#eventDetailsResepsi').html(
                        (response.tanggal_resepsi ? formatDate(response.tanggal_resepsi) : 'Senin, 5 Januari 2025') + '<br>' +
                        'Pukul ' + (response.waktu_resepsi ? formatTime(response.waktu_resepsi)  :  '18:30')+ ' '   +
                        (response.zona_waktu_resepsi || 'WITA') + '<br>' +
                        (response.alamat_resepsi || 'Inviteku.com') + '<br>' +
                        (response.lokasi_resepsi || 'Jl. Kembang Gugur, Luwu Raya, Sulawesi Selatan')
                    );
                ...
                if (buttonName === 'submit_maps' && response) {
                    ...
                    $('#mapsText1').text(response.maps_text_1 || 'Gedung: Serba Guna');
                    $('#mapsText2').text(response.maps_text_2 || 'Jl. Kembang Gugur, Luwu Raya, Sulawesi Selatan, Indonesia');
                ...
                if (buttonName === 'submit_gallery' && response) {
                    // Update HTML elements with the new data
                    $('#galleryText1').text(response.gallery_text_1 || 'Our Memories');
                ...
                if (buttonName === 'submit_rsvp' && response) {
                    $('#rsvpText1').text(response.rsvp_text_1 || 'RVSP');
                    $('#rsvpText2').text(response.rsvp_text_2 || '"Semoga Allah memberkahimu di waktu bahagia dan memberkahimu di waktu susah, dan mengumpulkan kalian berdua dalam kebaikan"');
                    $('#rsvpText3').text(response.rsvp_text_3 || '[HR. Abu Daud]');
                    $('#rsvpText4').text(response.rsvp_text_4 || 'Konfirmasi dan Kirim Ucapan');
                ...
                if (buttonName === 'submit_gift' && response) {
                    ...
                    $('#giftText1').text(response.gift_text_1 || 'Terima kasih telah menambah semangat kegembiraan pernikahan kami dengan kehadiran dan hadiah indah Anda.');
                ...
                if (buttonName === 'submit_thanks' && response) {
                    // Update HTML elements with the new data
                    $('#thanksText1').text(response.thanks_text_1 || 'Merupakan suatu kehormatan dan kebahagiaan bagi kami apabila Bapak/Ibu/Saudara (i) berkenan hadir untuk memberikan doa restu kepada kedua mempelai');
                    $('#thanksText2').text(response.thanks_text_2 || 'Hormat Kami yang mengundang');
                    $('#thanksText3').text(response.thanks_text_3 || 'Ssan & Aqila');
                ...
                if (buttonName === 'submit_gallery_image_1' && response) {
                    // Update HTML elements with the new data
                    const defaultImage = '/media/logo/blank-pic-removebg.png';
                ...
                if (buttonName === 'submit_gallery_image_2' && response) {
                    // Update HTML elements with the new data
                    const defaultImage = '/media/logo/blank-pic-removebg.png';
                ...
                if (buttonName === 'submit_gallery_image_3' && response) {
                    // Update HTML elements with the new data
                    const defaultImage = '/media/logo/blank-pic-removebg.png';
                ...
                if (buttonName === 'submit_gallery_image_4' && response) {
                    // Update HTML elements with the new data
                    const defaultImage = '/media/logo/blank-pic-removebg.png';
                ...
                if (buttonName === 'submit_gallery_image_5' && response) {
                    // Update HTML elements with the new data
                    const defaultImage = '/media/logo/blank-pic-removebg.png';
                ...
                if (buttonName === 'submit_gallery_image_6' && response) {
                    // Update HTML elements with the new data
                    const defaultImage = '/media/logo/blank-pic-removebg.png';
                ...
                if (buttonName === 'submit_gallery_image_7' && response) {
                    // Update HTML elements with the new data
                    const defaultImage = '/media/logo/blank-pic-removebg.png';
                ...
                if (buttonName === 'submit_gallery_image_8' && response) {
                    // Update HTML elements with the new data
                    const defaultImage = '/media/logo/blank-pic-removebg.png';
                ...
                if (buttonName === 'submit_gallery_image_9' && response) {
                    // Update HTML elements with the new data
                    const defaultImage = '/media/logo/blank-pic-removebg.png';
                ...
                if (buttonName === 'submit_gallery_image_10' && response) {
                    // Update HTML elements with the new data
                    const defaultImage = '/media/logo/blank-pic-removebg.png';
